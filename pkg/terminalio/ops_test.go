...
func Test_AddFileToDotfiles_unknown_path_gives_error(t *testing.T) {
	...
	actual := AddDotfile(file, userspacefile, dotfilesdir)
...
func Test_AddFileToDotfiles_successfully_copies_file_creates_symlink(t *testing.T) {
	...
	// Function under test
	err := AddDotfile(userspaceFile.Path, userspacedir.Path, dfilesdir.Path)
	if err != nil {
		test.Fail(err, "No error should have happened", t)
	}

	expectedDotfilesFile := filepath.Join(dfilesdir.Path, userdirpath, filepath.Base(userspaceFile.Path))
...
func Test_CopyDotfile_successfully_copies_file_into_dotfiles(t *testing.T) {
	env := test.NewTestEnvironment()
	defer env.Cleanup()
	...
	// assert on the returned path
	if dst != expectedDotfilesFile {
		t.Errorf("File in dotfiles dir should exist at %s", expectedDotfilesFile)
	}
